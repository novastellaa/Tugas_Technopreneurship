...
<div class="row mt-3">
    ...
    <form class="d-flex" method="POST">
        <input name="cari" class="form-control me-2" type="text" placeholder="Cari disini...">
          <input type="submit" name="btnCari" class="btn btn-danger" value="Cari">
    </form>
    </div>
</div>
  <div class="row mt-3 min-vh-100">
    ...
    <a class="btn btn-danger w-100 mb-3" type="submit" href="katalog.php?search=">Semua Kategori</a>
    <?php
        $kategori = mysqli_query($conn, "SELECT * FROM kategori");
        while ($row = mysqli_fetch_array($kategori)){
    ?>
        <a class="btn btn-danger w-100 mb-3" type="submit" href="katalog.php?search=<?php echo $row['nama_kategori']?>"><?php echo $row['nama_kategori']?></a>
    <?php
        }
    ?>
    </div>
